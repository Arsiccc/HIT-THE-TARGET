...
const WebGLUtils = {
	...
	async loadShaderSrc(path) {
		...
		if (!response.ok) {
			console.error(`Fai	led to load shader source from ${path}:`, response.statusText);
			return null;
		}
	...
	_compileShader(gl, shaderSrc, type) {
		...
		if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
			console.error(`Shader com	 if (!gl) return;

	   gl.enable(gl.DEPTH_TEST);pile failed (${shaderSrc}):\n`, gl.getShaderInfoLog(shader));
